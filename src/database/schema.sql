-- Document Verification API Database Schema
-- PostgreSQL schema for anti-fraud document processing system
...
CREATE TABLE processing_logs (
  ...
  stage VARCHAR(100) NOT NULL, -- 'upload_validation', 'ocr_extraction', 'data_comparison', etc.
...
COMMENT ON TABLE documents IS 'Main table storing document information and processing results';
COMMENT ON TABLE processing_logs IS 'Detailed logs of document processing stages';
COMMENT ON TABLE api_keys IS 'API authentication keys with rate limiting';
COMMENT ON TABLE request_logs IS 'HTTP request logging for monitoring and analytics';

COMMENT ON COLUMN documents.extracted_data IS 'OCR extracted data in JSON format';
COMMENT ON COLUMN documents.comparison_results IS 'Database comparison results in JSON format';
COMMENT ON COLUMN documents.authenticity_score IS 'AI authenticity verification score (0.0-1.0)';
COMMENT ON COLUMN documents.authenticity_details IS 'Detailed AI analysis results in JSON format';